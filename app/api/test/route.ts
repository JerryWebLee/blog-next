...
async function runTests() {
  ...
  try {
    ...
    results.postRetrieval = posts.length > 0;

    // 测试获取单篇文章
    await testGetPostById(post.id);

    // 测试更新文章
    const updatedPost = await testUpdatePost(post.id);
    results.postUpdate = updatedPost !== null;

    // 测试更新文章状态
    const statusUpdatedPost = await testUpdatePostStatus(post.id);
    results.postStatusUpdate = statusUpdatedPost !== null;

    // 测试增加浏览量
    const viewCountPost = await testIncrementViewCount(post.id);
    results.viewCountIncrement = viewCountPost !== null;
...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json({
      ...
      message: `测试完成: ${successCount}/${totalCount} 项测试通过`,
      results,
    });
  } catch (error) {
    console.error("\n💥 测试执行失败:", error);
