...
import { ApiResponse, PaginatedResponseData, Tag, CreateTagRequest, UpdateTagRequest, TagQueryParams } from "@/types/blog";
...
export function useTags(options: UseTagsOptions = {}): UseTagsReturn {
   ...
   * 获取标签列表
   */
  const fetchTags = useCallback(async (params?: Partial<TagQueryParams>) => {
    setLoading(true);
    setError(null);
   ...
   * 更新标签
   */
  const updateTag = useCallback(async (id: number, data: UpdateTagRequest): Promise<Tag | null> => {
    setLoading(true);
    setError(null);
